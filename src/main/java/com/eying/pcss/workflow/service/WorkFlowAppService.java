...
public class WorkFlowAppService {
     ...
     * @return
     */
    public List<WorkFlowApp> getApps(String companyId, String name, String groupId) {
    ...
    public String createGroupApp(WorkFlowAppBO workFlowAppBO) {
        ...
        if (StringUtils.isNotBlank(workFlowAppBO.getId())) {
        ...
        if (StringUtils.isNotBlank(workFlowAppBO.getGroupId())) {
            FlowGroup group = groupService.getFlowGroup(workFlowAppBO.getGroupId());
            if (group == null) {
        ...
        return workFlowAppDao.save(workFlowApp).getId();
    }
     ...
     * @return
     */
    public WorkFlowApp getApp(String id) {
     ...
     * @param app
     */
    public void editApp(String id, WorkFlowAppBO app) {
        ...
        if (StringUtils.isNotBlank(app.getGroupId())) {
            FlowGroup group = groupService.getFlowGroup(app.getGroupId());
            if (group == null) {
                throw new ServiceException(GenExceptCode.Request_Param.name(), FlowGroup.class.getSimpleName());
            }
            workFlowApp.setGroup(group);
    ...
    public void deleteApp(String id) {
        ...
        // 删除应用相关所有的审批单
        Predicate predicate = QFlowSheet.flowSheet.flowAppId.eq(id);
     ...
     * @param inputs
     */
    public void editAppForm(String id, List<FlowFormInput> inputs) {
        WorkFlowApp workFlowApp = workFlowAppDao.findById(id).orElse(null);
        if (workFlowApp == null) {
            throw new ServiceException(GenExceptCode.Request_Param);
        }
    ...
    public String publish(String id) {
        ...
        //校验应用分组
        if (StringUtils.isNotBlank(workFlowApp.getGroup().getId())) {
     ...
     */
    public void editAppFlow(String id, FlowDefinitionBO flowDefinition) {
        WorkFlowApp workFlowApp = workFlowAppDao.findById(id).orElse(null);
        if (workFlowApp == null) {
            throw new ServiceException(GenExceptCode.Request_Param);
        }
